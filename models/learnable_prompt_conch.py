...
class PromptLearner(nn.Module):
    def __init__(self, n_ctx=0, ctx_init="", all_ctx_trainable=True, csc=True, classnames=["Negative", "Positive"], clip_model='RN50', p_drop_out=0.0):
        ...
        ctx_dim = clip_model.text_decoder.ln_final.weight.shape[0]
        ...
        if ctx_init:
            if not self.use_class_specific_ctx:
                ...
                prompt = tokenize(texts=ctx_init, tokenizer=_tokenizer).cuda()
                with torch.no_grad():
                ...
                for ctx_init_i in ctx_init:
                    ...
                    prompt_i = tokenize(texts=[ctx_init_i], tokenizer=_tokenizer).cuda()
                    with torch.no_grad():
                        embedding_i = clip_model.text.token_embedding(prompt_i)
        ...
        tokenized_prompts = torch.cat([tokenize(texts=[p], tokenizer=_tokenizer).cuda() for p in prompts])
        with torch.no_grad():
            embedding = clip_model.text.token_embedding(tokenized_prompts)
        ...
        if not self.use_class_specific_ctx:
            ...
            for i in range(n_cls):
                CLS_pos = torch.where(tokenized_prompts[i] == 7281)[0].max()  # find last character "*"
...
class TextEncoder(nn.Module):
    def __init__(self, clip_model):
        ...
        self.pad_id = clip_model.pad_id
        self.heads = clip_model.heads
        self.output_tokens = clip_model.output_tokens
    ...
    def forward(self, prompts, tokenized_prompts):
        ...
        if self.cls_emb is not None:
            ...
            x = self.ln_final(x)
            pooled, tokens = x[torch.arange(x.shape[0]), tokenized_prompts.argmax(dim=-1)], x
...
class Instance_CLIP(nn.Module):
    def __init__(self, prompt_learner, clip_model="RN50"):
        ...
        self.logit_scale = clip_model.logit_scale
        self.logit_scale.requires_grad = False
        self.dtype = clip_model.text.positional_embedding.dtype
