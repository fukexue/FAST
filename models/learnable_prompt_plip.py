...
class PromptLearner(nn.Module):
    def __init__(self, n_ctx=0, ctx_init="", all_ctx_trainable=True, csc=True, classnames=["Negative", "Positive"], clip_model='RN50', p_drop_out=0.0):
        ...
        clip_model = PLIP('vinid/plip')
        dtype = clip_model.model.dtype
        ctx_dim = clip_model.model.visual_projection.weight.shape[0]
        ...
        if ctx_init:
            if not self.use_class_specific_ctx:
                ...
                prompt = clip_model.preprocess.tokenizer(ctx_init)
                with torch.no_grad():
                ...
                for ctx_init_i in ctx_init:
                    ...
                    # prompt_i = clip_model.preprocess(text=ctx_init_i, return_tensors="pt", max_length=77, padding="max_length", truncation=True).data['input_ids']
                    prompt_i = clip.tokenize(ctx_init_i)
                    # prompt_i = clip_model.preprocess.tokenize(ctx_init_i)
                    with torch.no_grad():
                        embedding_i = clip_model.model.text_model(prompt_i.cuda())['last_hidden_state']
        ...
        name_lens = [len(clip_model.preprocess.tokenizer(name)) for name in classnames]
        if not self.use_class_specific_ctx:
            prompts = [prompt_prefix + " " + name + "." for name in classnames]
        else:
            prompts = [prompt_prefix_i + " " + name + "." for prompt_prefix_i, name in zip(prompt_prefix, classnames)]

        # tokenized_prompts = torch.cat([clip_model.preprocess.tokenizer(p) for p in prompts])
        tokenized_prompts = torch.cat([clip_model.preprocess(text=p, return_tensors="pt", max_length=77, padding="max_length", truncation=True).data['input_ids'] for p in prompts])
        with torch.no_grad():
            # embedding = clip_model.token_embedding(tokenized_prompts).type(dtype)
            embedding = clip_model.model.text_model(tokenized_prompts.cuda())['last_hidden_state']
...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts):
        ...
        x = x[torch.arange(x.shape[0]), tokenized_prompts.argmax(dim=-1)] @ self.text_projection.weight
...
class MIL_CLIP(nn.Module):
    ...
    def forward(self, image_raw):
        ...
        if self.pooling_strategy == 'NoCoOp':  # special case for experiments, equivalent to baseline linear-probe(ABMIL)
            logits, _, atten_socre = self.pooling(image_raw.squeeze())
...
class Instance_CLIP(nn.Module):
    def __init__(self, prompt_learner, clip_model="RN50"):
        super().__init__()
        self.prompt_learner = prompt_learner

        clip_model = PLIP('vinid/plip').model
        ...
        self.coord_trans = nn.Sequential(
            nn.Linear(self.prompt_learner.tokenized_prompts.shape[0], 1),
        )
    ...
    def forward(self, image_features):
        instance_prompts = self.prompt_learner()
        instance_text_features = self.text_encoder(instance_prompts, self.prompt_learner.tokenized_prompts)
        instance_text_features = instance_text_features / instance_text_features.norm(dim=-1, keepdim=True)

        logit_scale = self.logit_scale.exp()
        logits = logit_scale * image_features @ instance_text_features.t()
        return logits
