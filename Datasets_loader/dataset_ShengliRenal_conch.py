...
def statistics_slide(slide_path_list, pos_region_threshold=0.5):
    ...
    for i in tqdm(slide_path_list, desc="Statistics"):
        ...
        num_pos_patch = 0
        for j in glob.glob(i + "/*_Tumor.jpg"):
...
class ShengliRenal_5x(torch.utils.data.Dataset):
    # @profile
    def __init__(self, root_dir='/home/science/code/python3/Data/ShandongShengli/Renal_crop',
        ...
        for i in tqdm(all_slides, ascii=True, desc='preload data'):
            for j in os.listdir(i):
                ...
                self.patch_label.append(int('Tumor' in j))
                self.patch_corresponding_slide_label.append(int('Tumor' in i.split('/')[-1]))
    ...
    def __getitem__(self, index):
        if self.return_bag:
            ...
            # patch_image_copy = patch_image.copy()
...
class ShengliRenal_5x_feat(torch.utils.data.Dataset):
    # @profile
    def __init__(self, root_dir='/home/science/code/python3/plip/output_ShengliRenal_feat_224x224_5x_CONCH',
...
def cal_img_mean_std():
    train_ds = ShengliRenal_5x(train=True, transform=None, downsample=1.0, return_bag=False)
...
def camelyon16_dataloader(only_return_valloader=False, batch_size=8):
    if only_return_valloader:
        val_ds = ShengliRenal_5x(train=False, downsample=1.0, return_bag=False)
        val_loader = torch.utils.data.DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=0, drop_last=False, pin_memory=True)
        return val_loader
    else:
        train_ds = ShengliRenal_5x(train=True, transform=transform_data, downsample=1.0, return_bag=False)
        val_ds = ShengliRenal_5x(train=False, downsample=1.0, return_bag=False)
        train_loader = torch.utils.data.DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=0, drop_last=False, pin_memory=True)
        val_loader = torch.utils.data.DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=0, drop_last=False, pin_memory=True)
...
if __name__ == '__main__':
    ...
    train_ds = ShengliRenal_5x(train=True, transform=transform_data, downsample=0.01, return_bag=False)
    val_ds = ShengliRenal_5x(train=False, transform=transform_data, downsample=0.01, return_bag=False)
